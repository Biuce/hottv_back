    ...
    <div class="panel">
        ...
        <section class="section-sm bg-800">
            <div class="container-fluid">
                ...
                <div class="media-item">
                    @if($info['is_cancel'] == 0)
                        <a class="media-item" href="javascript:;"
                           data-modal-trigger='{"target":"#modal-sample"}'>
                            <button class="btn btn-primary">{{ trans('adminUser.up_remark') }}</button>
                        </a>
                        @if($type == 2 || \Auth::guard('admin')->user()->id == 1)
                            <a class="media-item" style="margin-left: 20px;"
                               href="javascript:;">
                                <button class="btn btn-primary"
                                        disabled="disabled">{{trans('adminUser.set_level')}}</button>
                            </a>
                        @else
                            <a class="media-item" style="margin-left: 20px;"
                               href="{{ route('admin::adminUser.level', ['id' => $info['id']]) }}">
                                <button class="btn btn-primary">{{trans('adminUser.set_level')}}</button>
                            </a>
                        @endif
                        @if($info['level_id'] == 8)
                            <a class="media-item" style="margin-left: 20px;"
                               href="{{ route('admin::adminUser.cost', ['id' => $info['id']]) }}">
                                <button class="btn btn-primary">{{trans('adminUser.cost')}}</button>
                            </a>
                        @endif
                    @endif
                    <span style="margin-left: 80px;">{{trans('adminUser.balance')}}
                    ...
                    @if(\Auth::guard('admin')->user()->id == 1)
                        <span style="margin-left: 40px;">{{trans('adminUser.add_profit')}}
                            : {{ number_format($user_pro, 2) }}</span>
                    @else
                        <span style="margin-left: 40px;">{{trans('adminUser.add_profit')}}
                            : {{ number_format($profit, 2) }}</span>
                    @endif
                    @if(\Auth::guard('admin')->user()->id == 1)
                        <span style="margin-left: 40px;">{{trans('adminUser.last_profit')}}
                            : {{ number_format($profit_time, 2) }}</span>
                    @endif
                </div>
                {{--@endif--}}
            </div>
                            ...
                            @if(\Auth::guard('admin')->user()->id == 1)
                                <div style="margin-top: 20px;">
                                    <form name="admin_list_sea" class="form-search" method="post"
                                          action="{{ route('admin::adminUser.export') }}" id="export">
                                        {{ csrf_field() }}
                                        ...
                                        <div class="row row-15">
                                            <div class="col-sm-2">
                                                <div class="input-group">
                                                    <input class="form-control" type="text" name="month" id="date2"
                                                           value="" autocomplete="off"
                                                           placeholder="{{trans('general.export_excel')}}">
                                                </div>
                                            </div>

                                            <div class="col-sm-2">
                                                <button class="btn btn-success" type="submit" lay-submit
                                                        lay-filter="formAdminUser"
                                                        id="submitBtn">{{trans('general.excel')}}</button>
                                            </div>
                                        </div>
                                    </form>
                                </div>
            ...
            <div class="row row-10 align-items-center">
                <div class="col-sm-12 text-sm-center">
                    ...
                    <button type="button" class="btn btn-warning"
                            id="backurl"
                            onclick="window.location = '{{ route('admin::adminUser.index') }}'">{{trans('general.return')}}</button>
                    {{--</div>--}}
                </div>
            </div>
        </div>
