    ...
    <div class="panel">
        <div class="panel-header">
            <div class="d-flex flex-wrap align-items-center justify-content-between group-5">
                <h4 class="panel-title">
                    ...
                    <span>{{trans('authCode.managers')}}</span>
                </h4>
            ...
            <div style="margin-top: 20px;">
                <form name="admin_list_sea" class="form-search" method="get" id="export" action="{{ route('admin::code.index') }}">
                    {{ csrf_field() }}
                    <div class="row row-15">
                        <div class="col-sm-2">
                            <div class="input-group">
                                <input class="form-control" id="text" type="text"
                                       placeholder="{{trans('authCode.auth_code')}}" name="auth_code"
                        ...
                        <div class="col-sm-2">
                            <select class="form-control" name="assort_id">
                                <option value="-1">{{ trans('general.select_code') }}</option>
                                @foreach($assort_list ?? null as $key => $v)
                                    <option value="{{ $v['id'] }}"
                                            @if(isset($condition['assort_id']) && $condition['assort_id'] == $v['id']) selected @endif>{{ $v['assort_name'] }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-sm-3">
        ...
        <div class="panel-body">
            <div class="table-responsive">
                <table class="table table-striped table-hover" style="padding-bottom: 20px;">
                    ...
                    <tr class="long-tr">
                        ...
                        <th>{{trans('authCode.auth_code')}}</th>
                        <th>{{trans('authCode.code_func')}}</th>
            ...
            <div id="pages" style="margin-top: 30px;margin-bottom: -50px;">
                {!! $lists->appends(['auth_code'=>$lists->auth_code, 'status'=>$lists->status, 'assort_id'=>$lists->assort_id, 'date2'=>$lists->date2])->render() !!}
            </div>
        </div>
    </div>
</div>
...
@section('js')
    <script>
        ...
        function check(id) {
            var url = "{{ route('admin::code.index') }}";
            var excel_url = "{{ route('admin::code.export') }}";
            if (id == 1) {
                console.log(url);
